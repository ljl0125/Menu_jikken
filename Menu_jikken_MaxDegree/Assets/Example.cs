...
public class Example : MonoBehaviour
{
    ...
    public string path = @"D:\Menu_jikken\Menu_jikken\Menu_jikken_MaxDegree\Data\Data06.csv";
    ...
    void Update()
    {
        ...
        else if (m_joyconL.GetButtonUp(Joycon.Button.SHOULDER_2))
        {
            ...
            if (Cursor_Test.flag_Yes)
            {
                Cursor_Test.count_yes++;
                //Debug.Log("Yes:" + Cursor_Test.count_yes + ",now_num:" + Cursor_Test.now_num);
                Cursor_Test.flag_Yes = false;
            }

            if (Cursor_Test.flag_All)
            {
                Cursor_Test.count_all++;
                Debug.Log("Yes:" + Cursor_Test.count_yes + ",now_name:" + Convert.ToInt32(Cursor_Test.now_name));
                Debug.Log("All:" + Cursor_Test.count_all);
                Cursor_Test.flag_All = false;
                flag_count_all = false;
                ...
                if(Cursor_Test.count_all % 2 == 1)
                {
                    string[] s1 = { ((Cursor_Test.count_all + 1) / 2).ToString(), Cursor_Test.now_name };
                    string s2 = string.Join(",", s1);
                    sw.Write(s2);
                }
                else
                {
                    string s1 = "," + (360 - Convert.ToInt32(Cursor_Test.now_name)).ToString();
                    sw.WriteLine(s1);
                }
                // StreamWriterを閉じる
                sw.Close();
